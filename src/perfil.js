        ...
        const API_BASE_URL = 'https://api-restaurante-1-6u7x.onrender.com/api';
        ...
        async function carregarDadosUsuario() {
            try {
                ...
                const response = await fetch(`${API_BASE_URL}/user/check-session?userName=${username}`);
        ...
        async function alterarSenha() {
            ...
            try {
                const username = localStorage.getItem('usuario') || 'TridonJ';
                const response = await fetch(`${API_BASE_URL}/user/alterar-senha?Usuario=${username}&senhaAntiga=${currentPassword}&senhaNova=${newPassword}`, {
                    method: 'POST'
                });
